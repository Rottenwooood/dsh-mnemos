...
1. **效果数字可复跑、超过同口径对手。** LongMemEval-S hit@1 **87.2%**，deja-vu 官方公布 **85.3%**——同数据、同指标、同问题原文。自己跑一遍（见[效果](#效果)），确定性的、不用 LLM。
2. **有完整的记忆生命周期。** 活跃 → 归档 → 可还原，按热度清理（冷的最先）；`固定` 的记忆永不离开。事实被新事实取代时新旧都保留并接上**接替链**——召回时新值在前、旧值标注"已被取代"。
3. **记得住教训，不只是事实。** 失败的命令被记录，一模一样再来直接拦下并告诉模型为什么；成功重试自动解除。
4. **记忆即代码。** 一条记忆一个 Markdown 文件，进 git 仓库：逐条历史、diff、回滚、已删除恢复、跨机合并同步、标准 bundle 备份。默认纯 JS 的 git 后端（`isomorphic-git`），不需要系统 git。
...
- **跨机同步与备份。** git 远端（复用 `~/.git-credentials`）合并式同步；冲突标记交人工裁决，绝不静默覆盖；标准 `git bundle` 导出备份。
...
deja-vu 是 Go 写的记忆引擎，它的公开长期记忆基准我们用同口径复现。LongMemEval-S 我们赢（87.2% vs 85.3%）、LoCoMo 落后（60.9% vs 69.6%），细节见[效果](#效果)。在检索之上，我们还带了 deja-vu 没有的治理/生命周期层（审批门禁、信任分级、接替链、负面记忆、git）。
